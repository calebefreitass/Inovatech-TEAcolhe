...
import ConnectionScreenUser from '../pages/ConnectionScreenUser';
import ConnectionScreenTutor from '../pages/ConnectionScreenTutor';
...
// --- 1. ABAS DO TUTOR (Foco: Mapa e Gestão) ---
function TutorTabs() {
    return (
...
// --- 2. ABAS DO USUÁRIO (Foco: SOS e Rotina) ---
function UserTabs() {
    return (
        <SafeAreaView style={{ flex: 1 }} edges={['bottom']}>
            <Tab.Navigator
...
// Estilização comum para as abas
const tabScreenOptions = ({ route }) => ({
    ...
    tabBarIcon: ({ focused, color, size }) => {
        ...
        if (route.name === 'Início') { // Home do Usuário
            iconSource = require('../../assets/icon/home-icon.png');
            isImage = true; 
        } 
        else if (route.name === 'Localização') { // Home do Tutor
...
export default function Routes() {
    ...
    // Lógica de Conexão (Telas de bloqueio se não houver vínculo)
    if (userData?.tipo === 'USUARIO_TEA' && !userData?.tutorConectado) {
        return <ConnectionScreenUser />;
    } else if (userData?.tipo === 'TUTOR' && !userData?.usuarioConectado) {
        return <ConnectionScreenTutor />;
    }

    // DECISÃO FINAL: Qual conjunto de abas mostrar?
